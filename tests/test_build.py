...
class BuildTests(unittest.TestCase):
    ...
    def test_activity_radar_renders_official_events_and_interviews(self):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn('发布会 / 播客 / 视频雷达',html)
            ...
            self.assertIn('播客 / 视频源每 2 小时检查',html)
            self.assertIn('值得看的发布会',html)
            self.assertIn('播客精选',html)
            self.assertIn('YouTube 优秀视频',html)
            ...
            self.assertIn("function v2ActivitySourceHref(type,id)",html)
            self.assertIn("v2ActivitySourceFits(type,s)",html)
            self.assertIn("activeType==='all'",html)
