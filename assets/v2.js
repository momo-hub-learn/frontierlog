...
function v2Progress(){
 ...
 const unresolved=DATA.items;
 const item=(t,mode)=>`<button class="v2-cap-row" data-action="task" data-id="${t.id}"><span class="v2-cap-state ${mode}"></span><span><strong>${esc(t.title)}</strong><small>${esc(t.name)} · ${esc(t.category)}</small></span><span class="v2-cap-copy">${esc(mode==='open'?t.summary:mode==='research'?t.scope:t.boundary)}</span>${icon('arrow')}</button>`;
 $('#content').innerHTML=`<div class="v2-pageintro"><div><p class="eyebrow">WHAT AI CAN ACTUALLY DO</p><h1>不是“会不会”，而是走到哪一步。</h1><p>公开实现、研究推进和仍需验证的边界分开看。</p></div><span class="v2-pagecount">${DATA.items.length} 个任务</span></div>
 <div class="v2-cap-status-grid">
  <section><header><span class="v2-status-dot open"></span><div><h2>已经可用</h2><p>有公开实现或可运行路径，不等于业务已验收。</p></div><b>${usable.length}</b></header><div>${usable.map(t=>item(t,'open')).join('')}</div></section>
  <section><header><span class="v2-status-dot research"></span><div><h2>正在推进</h2><p>已有研究证据，但开放范围或稳定性仍有限。</p></div><b>${research.length}</b></header><div>${research.map(t=>item(t,'research')).join('')||'<p class="v2-emptyline">暂无单独研究阶段任务。</p>'}</div></section>
  <section class="wide"><header><span class="v2-status-dot unresolved"></span><div><h2>仍未解决</h2><p>每项能力真正进入生产前，还卡在哪里。</p></div><b>${unresolved.length}</b></header><div class="v2-boundary-grid">${unresolved.map(t=>item(t,'unresolved')).join('')}</div></section>
 </div>`;
}
