...
class BuildTests(unittest.TestCase):
    ...
    def test_activity_radar_renders_frontier_live_categories(self):
        radar=json.loads((ROOT/'data/activity-radar.json').read_text())
        sources={x['id']:x for x in radar['sources']}
        self.assertTrue(any(x['status']=='upcoming' for x in radar['events']))
        self.assertTrue({'dwarkesh','latent-space','no-priors','lex-fridman','zhang-xiaojun','silicon-valley-101','42zhangjing','crossing'} <= set(sources))
        self.assertTrue({'openai-engineering','anthropic-engineering','google-research-blog','nvidia-technical-blog','microsoft-research-blog','meta-ai-research'} <= set(sources))
        self.assertTrue(all(sources[x]['authority']=='official' for x in ['openai-engineering','anthropic-engineering','google-research-blog','nvidia-technical-blog','microsoft-research-blog','meta-ai-research']))
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn('AI 前沿现场',html)
            self.assertIn('官方发布',html)
            self.assertIn('技术深读',html)
            self.assertIn('深度对谈',html)
            self.assertIn("id:'talks',title:'技术演讲'",html)
            self.assertIn("id:'demo',title:'实战 Demo'",html)
            self.assertNotIn("id:'youtube',title:'YouTube 优秀视频'",html)
            self.assertIn('OpenAI Engineering',html)
            self.assertIn('Anthropic Engineering',html)
            self.assertIn('Google Research Blog',html)
            self.assertIn('张小珺Jùn｜商业访谈录',html)
            self.assertIn('硅谷101',html)
            ...
            self.assertIn("v2ActivitySourceFits(type,s)",html)
            self.assertIn("type==='technical'",html)
