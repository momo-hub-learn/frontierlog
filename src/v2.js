...
function v2Activity(){
 ...
 $('#content').innerHTML=`<div class="v2-pageintro v2-activity-intro"><div><h1>发布会 / 访谈雷达</h1><p>官方发布会看方向，一手访谈听大佬怎么想；项目兑现单独追，不把观点当成已发布事实。</p><small>点上方来源 Tab 直接筛选；访谈源每 2 小时检查。论文正式录用 / 发表后才标 CCF A/B/C、Nature / Science / 顶刊；arXiv 只标“预印本”，不把投稿或预印本升级成录用。</small></div><span class="v2-pagecount">${activeSource==='all'?sources.length+' 个高信号源':esc(activeLabel)+' · '+visibleCount+' 条'}</span></div>
 ${sourceTabs}
