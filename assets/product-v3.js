...
function decorateBaseModelCompanies(){
 document.querySelectorAll('.m-rowname .m-rowmeta').forEach(el=>{
  if(el.dataset.companyLinked==='1')return;
  const raw=(el.textContent||'').trim(),maker=raw.split(' · ')[0],c=companyFor(maker);
  if(!c)return;
  const suffix=raw.slice(maker.length);
  el.innerHTML=companyAnchor(maker,c.short||maker,'model-maker-link')+esc(suffix);
  el.dataset.companyLinked='1';
 });
}
